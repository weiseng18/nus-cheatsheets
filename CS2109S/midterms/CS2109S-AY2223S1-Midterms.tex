...
\begin{multicols*}{3}
    \subsection*{Tree search} \noindent
      \includegraphics[width=\columnwidth]{L2/tree-search} 
      \begin{itemize}[leftmargin=*]
        \item May revisit nodes
        \item Checks for goal state \textbf{after} traversing to the node
      \end{itemize}
    \subsection*{Graph search} \noindent
      \includegraphics[width=\columnwidth]{L2/graph-search}
      \begin{itemize}[leftmargin=*]
  ...
  \section*{Informed Search}
    ...
    \subsection*{Memory-bounded heuristic search}
      ...
      \paragraph{Simplified MA$^*$}
        \begin{itemize}[leftmargin=*]
          \item Run $A^*$ until memory full
          \item When memory full
            \begin{itemize}[leftmargin=*]
    ...
    \subsection*{Local search}
      ...
      \paragraph{Hill climbing}
        ...
        \end{itemize}
        \includegraphics[width=\columnwidth]{L3/hill-climbing}
      \paragraph{Simulated annealing}
        ...
        \end{itemize}
        \includegraphics[width=\columnwidth]{L3/simulated-annealing}
...
\section*{Adversarial Search}
  \subsection*{Minimax}
    ...
    \end{itemize}
    \includegraphics[width=\columnwidth]{L4/minimax}
  ...
  \subsection*{Decision trees}
    ...
    \paragraph{Lecture Training}
      \begin{itemize}[leftmargin=*]
        ...
        \item Can use binary or discrete or binned/discretized continuous valued attributes, but not continuous valued attributes
      \end{itemize}
    \includegraphics[width=\columnwidth]{L4/decision-tree}
    \paragraph{Entropy}
      \begin{itemize}[leftmargin=*]
        \item Measure ``impurity'' or ``randomness'' in data
        \item Defined as
          \[ I(P(v_1), \cdots, P(v_n)) = - \sum_{i=1}^n P(v_i) \log_2 P(v_i) \]
...
\section*{Linear regression} \noindent
  ...
  \subsection*{Gradient descent}
    \begin{itemize}[leftmargin=*]
      ...
      \item In implementation, learning rate $\alpha$ often decreases with more iterations
        \begin{itemize}[leftmargin=*]
          \item Start big to reach optimal values faster
          \item Slow down to allow for gradual convergence
          \item Idea behind a learning rate scheduler
        \end{itemize}
    \end{itemize}
  ...
  \subsection*{Multivariable gradient descent}
    \begin{itemize}[leftmargin=*]
      \item Hypothesis $h$ takes in a vector, returning a scalar
        ...
        or
        \[ h_w(x) = w^Tx \]
        where $w = \mat{w_0; w_1; \vdots; w_n}$ and $x = \mat{x_0; x_1; \vdots; x_n} = \mat{1; x_1; \vdots; x_n}$. The 1 introduced for the bias may be treated as another variable $x_0$.
      \item Cost function
  ...
  \subsection*{Feature scaling}
    \begin{itemize}[leftmargin=*]
      ...
      \item Perform mean normalization
  ...
  \subsection*{Normal equation} \noindent
    ...
    \paragraph{Derivation}
      \begin{align*}
        ...
        2m J(w) &= ((Xw)^T - Y^T) (Xw - Y) \\
                &= (Xw)^T(Xw) - (Xw)^T Y - Y^T(Xw) - Y^T Y \\
                &= w^T(X^T X)w - 2(Xw)^T Y + Y^T Y
      \end{align*}
      Differentiate and set to 0:
      \begin{align*}
        \frac{\partial J}{\partial w} = 2(X^T X)w - 2X^TY &= 0 \\
        (X^T X)w &= X^T Y \\
            w &= (X^T X)^{-1} X^T Y
      \end{align*}
\end{multicols*}
