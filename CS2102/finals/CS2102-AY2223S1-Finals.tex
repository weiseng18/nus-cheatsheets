...
\section*{Anomalies and BCNF}
  \subsection*{Anomalies} \noindent
    ...
    \begin{itemize}[leftmargin=*]
      ...
      \item \textbf{Update anomaly}: Two rows of the table have the same value for the column \ic{department} but different values for the column \ic{faculty}, violating the FD
  ...
  \subsection*{Decomposition}
    ...
    \subsubsection*{Check if decomposed set of relations is lossless-join}
      ...
      \end{enumerate}
    \subsubsection*{Check if decomposed set of relations is dependency-preserving}
      \begin{enumerate}[leftmargin=*]
        \item Let $X$ be a minimal cover of the original set of relations. Let $Y$ be the union of projected FDs to each relation
        \item For each FD in $X$, check that we can derive it in $Y$. If some FD could not be derived, then that FD was not preserved, then FALSE. Else TRUE
      \end{enumerate}
